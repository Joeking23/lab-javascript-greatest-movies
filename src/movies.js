...
// Define the moviesArray before calling the function
const moviesArray = [
    {
...
// function remove multiple directors
function removeMultipleDirectors(directorsArray) {
    const uniqueDirectors = [...new Set(directorsArray)];
    return uniqueDirectors;
}
const uniqueDirectors = removeMultipleDirectors(directors);
...
function howManyMovies(moviesArray) {
    ...
    // Return the number of movies that match the criteria
    return spielbergDramas.length;
}
const numberOfSpielbergDramas = howManyMovies(moviesArray);
console.log(numberOfSpielbergDramas);
...
const topTitles = orderAlphabetically(moviesArray);
...
const updatedMovies = turnHoursToMinutes(moviesArray);
console.log(updatedMovies);
// BONUS - Iteration 8: Best yearly score average - Best yearly score average
  function bestYearAvg(moviesArray) {
    if (moviesArray.length === 0) return "No movies available";

    // Create an object to store total scores and counts by year
    const scoresByYear = moviesArray.reduce((acc, movie) => {
        const year = movie.year;
        const score = movie.score;

        // Initialize year in the accumulator if it doesn't exist
        if (!acc[year]) {
            acc[year] = { totalScore: 0, count: 0 };
        }

        // Accumulate total score and count of movies for the year
    ...
    // Determine the best year with the highest average score
    let bestYear = null;
    let highestAverageScore = 0;

    for (const year in scoresByYear) {
        const { totalScore, count } = scoresByYear[year];
    ...
    return `The best year was ${bestYear} with an average score of ${highestAverageScore.toFixed(2)}`;
}
